...
int	in_circle(float x, float y, t_shape shape)
{
	...
	distance = sqrtf(powf(x - shape.xw, 2.) + powf(y - shape.yh, 2.));
	...
	while (y < env->h)
	{
		...
		while (x < env->w)
		{
			...
			if (is_it == IS_IN_BORDER
				|| (shape.type == 'C' &&  is_it == IS_IN_CENTER))
...
int	consume_circles(t_env *env, FILE *f)
{
	int res;
	t_shape shape;
	while (1)
	{
		res = fscanf(f, "%c %f %f %f %c\n", &shape.type, &shape.xw, &shape.yh, &shape.rad, &shape.c);
		if (res == -1)
			break;
		if (res != 5 || (shape.type != 'c' && shape.type != 'C') || shape.rad <= 0.0)
		{
			clear(f, env);
			return -EXIT_FAILURE;
		}
...
int main(int ac, char** av)
{
	...
	if (consume_circles(&env, f) != EXIT_SUCCESS)
		return err(MSG_FILE);
	print_area(&env);
	clear(f, &env);
	return 0;
